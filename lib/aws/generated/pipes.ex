...
defmodule AWS.Pipes do
  ...
  * `:target_prefix` (`t:string`) The prefix matching the pipe target.
  ...
  def list_pipes(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [current_state: nil, desired_state: nil, limit: nil, name_prefix: nil, next_token: nil, source_prefix: nil, target_prefix: nil
    # ])

    headers = []
    query_params = []

    {target_prefix, options} = Keyword.pop(options, :target_prefix, nil)
    ...
    {source_prefix, options} = Keyword.pop(options, :source_prefix, nil)
    ...
    {desired_state, options} = Keyword.pop(options, :desired_state, nil)
    ...
    {current_state, options} = Keyword.pop(options, :current_state, nil)
