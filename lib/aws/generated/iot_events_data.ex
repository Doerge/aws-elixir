...
defmodule AWS.IoTEventsData do
  ...
  *IoT Events Developer Guide*.
  ...
  ## Optional parameters:
   • :key_value (t:String.t/0) (keyValue)
  """
  ...
  def describe_alarm(%Client{} = client, alarm_model_name, options \\ []) do
    ...
    # options = Keyword.validate!(options, [key_value: nil
    # ])

    headers = []
    query_params = []

    {key_value, options} = Keyword.pop(options, :key_value, nil)
  ...
  ## Optional parameters:
   • :max_results (t:String.t/0) (maxResults)
   • :next_token (t:String.t/0) (nextToken)
   • :state_name (t:String.t/0) (stateName)
  """
  ...
  def list_detectors(%Client{} = client, detector_model_name, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [max_results: nil, next_token: nil, state_name: nil
    # ])

    headers = []
    query_params = []

    {state_name, options} = Keyword.pop(options, :state_name, nil)
