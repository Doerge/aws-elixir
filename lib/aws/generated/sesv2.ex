...
defmodule AWS.SESv2 do
  ...
  * `:blacklist_item_names` (`t:list[com.amazonaws.sesv2#BlacklistItemName]`) A
  ...
  def get_blacklist_reports(%Client{} = client, blacklist_item_names, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [blacklist_item_names: nil
    # ])

    headers = []
    query_params = []

    {blacklist_item_names, options} = Keyword.pop(options, :blacklist_item_names, nil)
  ...
  * `:pool_name` (`t:string`) The name of the IP pool that the dedicated IP
  ...
  def get_dedicated_ips(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [next_token: nil, page_size: nil, pool_name: nil
    # ])

    headers = []
    query_params = []

    {pool_name, options} = Keyword.pop(options, :pool_name, nil)
  ...
  * `:start_date` (`t:timestamp`) The first day (in Unix time) that you want to
  ...
  def get_domain_statistics_report(
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [end_date: nil, start_date: nil
    # ])
  ...
  * `:start_date` (`t:timestamp`) The first day that you want to obtain
  ...
  def list_domain_deliverability_campaigns(
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [end_date: nil, next_token: nil, page_size: nil, start_date: nil
    # ])
  ...
  * `:start_date` (`t:timestamp`) Used to filter the list of suppressed email
  ...
  def list_suppressed_destinations(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [end_date: nil, next_token: nil, page_size: nil, reasons: nil, start_date: nil
    # ])
    ...
    {reasons, options} = Keyword.pop(options, :reasons, nil)
