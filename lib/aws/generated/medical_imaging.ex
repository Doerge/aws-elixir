...
defmodule AWS.MedicalImaging do
  ...
  *AWS HealthImaging Developer Guide* where you can view tested code examples.
  ...
  def list_d_i_c_o_m_import_jobs(%Client{} = client, datastore_id, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [job_status: nil, max_results: nil, next_token: nil
    # ])
    ...
    {job_status, options} = Keyword.pop(options, :job_status, nil)
  ...
  def list_datastores(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [datastore_status: nil, max_results: nil, next_token: nil
    # ])
    ...
    {datastore_status, options} = Keyword.pop(options, :datastore_status, nil)
