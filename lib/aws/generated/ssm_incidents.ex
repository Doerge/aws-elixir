...
defmodule AWS.SSMIncidents do
  ...
  * `:incident_record_arn` (`t:string`) The Amazon Resource Name (ARN) of the
  ...
  def get_timeline_event(%Client{} = client, event_id, incident_record_arn, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [event_id: nil, incident_record_arn: nil
    # ])

    headers = []
    query_params = []

    {incident_record_arn, options} = Keyword.pop(options, :incident_record_arn, nil)
    ...
    {event_id, options} = Keyword.pop(options, :event_id, nil)
