...
defmodule AWS.WAFV2 do
  @moduledoc """
  WAF This is the latest version of the **WAF** API, released in November, 2019.
  ...
  APIs have retained the prior names, endpoints, and namespaces. For
  information, including how to migrate your WAF resources to this version, see
  the [WAF Developer
  ...
  Amazon Web Services resource that WAF is monitoring. Based on conditions that
  you specify, such as the IP addresses that requests originate from or the
  values of query strings, the protected resource responds to requests with
  either the requested content, an HTTP 403 status code (Forbidden), or with a
  custom response.
  """
  ...
  pool, an App Runner service, or an Amazon Web Services Verified Access
  instance. For Amazon CloudFront, don't use this call. Instead, use your
  ...
  * `:input` (`t:describe_managed_rule_group_request`)
  ...
  ACL association. A resource can have at most one web ACL association. A
  ...
  Runner service, or an Amazon Web Services Verified Access instance. For Amazon
  ...
  * `:input` (`t:get_web_acl_request`)
  ...
  Retrieves the `WebACL` for the specified resource. This call uses `GetWebACL`,
  to verify that your account has permission to access the retrieved web ACL. If
  ...
  * `:input` (`t:list_regex_pattern_sets_request`)
  ...
  resources that are associated with the specified web ACL. For Amazon
  ...
  * `:input` (`t:list_web_acls_request`)
  ...
  according to the configuration provided. This operation completely replaces
  any mutable specifications that you already have for a logging configuration
  with the ones that you provide to this call. To modify an existing logging
  ...
  * `:input` (`t:put_logging_configuration_request`)
  ...
  versioning. This is intended for use only by vendors of managed rule sets.
  Vendors are Amazon Web Services and Amazon Web Services Marketplace sellers.
  ...
  * `:input` (`t:update_managed_rule_set_version_expiry_date_request`)
  ...
  mutable specifications that you already have for the regex pattern set with
  ...
  * `:input` (`t:update_regex_pattern_set_request`)
  ...
  Updates the specified `RuleGroup`. This operation completely replaces the
  mutable specifications that you already have for the rule group with the ones
  ...
  * `:input` (`t:update_rule_group_request`)
  ...
  ACL. This operation completely replaces the mutable specifications that you
