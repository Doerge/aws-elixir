...
defmodule AWS.MarketplaceCatalog do
  ...
  * `:change_set_id` (`t:string`) Required. The unique identifier for the <code>StartChangeSet</code> request that you
  ...
  def describe_change_set(%Client{} = client, catalog, change_set_id, options \\ []) do
    ...
    # options = Keyword.validate!(options, [catalog: nil, change_set_id: nil
    # ])

    headers = []
    query_params = []

    {change_set_id, options} = Keyword.pop(options, :change_set_id, nil)
    ...
    {catalog, options} = Keyword.pop(options, :catalog, nil)
  ...
  * `:entity_id` (`t:string`) Required. The unique ID of the entity to describe.
  ...
  def describe_entity(%Client{} = client, catalog, entity_id, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [catalog: nil, entity_id: nil
    # ])

    headers = []
    query_params = []

    {entity_id, options} = Keyword.pop(options, :entity_id, nil)
