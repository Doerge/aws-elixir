...
defmodule AWS.Fis do
  ...
  * `:target_name` (`t:string`) The name of the target.
  ...
  def list_experiment_resolved_targets(%Client{} = client, experiment_id, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [max_results: nil, next_token: nil, target_name: nil
    # ])

    headers = []
    query_params = []

    {target_name, options} = Keyword.pop(options, :target_name, nil)
  ...
  * `:next_token` (`t:string`) The token for the next page of results.
  ...
  def list_experiments(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [experiment_template_id: nil, max_results: nil, next_token: nil
    # ])
    ...
    {experiment_template_id, options} = Keyword.pop(options, :experiment_template_id, nil)
