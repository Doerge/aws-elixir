...
defmodule AWS.Kafka do
  ...
  * `:cluster_arn` (`t:string`) The Amazon Resource Name (ARN) of the cluster check.
  ...
  def get_compatible_kafka_versions(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [cluster_arn: nil
    # ])
  ...
  * `:next_token` (`t:string`) The paginated results marker. When the result of the operation is truncated, the call returns NextToken in the response. 
  ...
  def list_clusters(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [cluster_name_filter: nil, max_results: nil, next_token: nil
    # ])
    ...
    {cluster_name_filter, options} = Keyword.pop(options, :cluster_name_filter, nil)
  ...
  * `:next_token` (`t:string`) The paginated results marker. When the result of the operation is truncated, the call returns NextToken in the response. 
  ...
  def list_clusters_v2(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [cluster_name_filter: nil, cluster_type_filter: nil, max_results: nil, next_token: nil
    # ])
    ...
    {cluster_type_filter, options} = Keyword.pop(options, :cluster_type_filter, nil)
  ...
  * `:replicator_name_filter` (`t:string`) Returns replicators starting with given name.
  ...
  def list_replicators(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [max_results: nil, next_token: nil, replicator_name_filter: nil
    # ])

    headers = []
    query_params = []

    {replicator_name_filter, options} = Keyword.pop(options, :replicator_name_filter, nil)
