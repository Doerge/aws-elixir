...
defmodule AWS.DataExchange do
  ...
  def list_data_sets(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [max_results: nil, next_token: nil, origin: nil
    # ])

    headers = []
    query_params = []

    {origin, options} = Keyword.pop(options, :origin, nil)
  ...
  def list_event_actions(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [event_source_id: nil, max_results: nil, next_token: nil
    # ])
    ...
    {event_source_id, options} = Keyword.pop(options, :event_source_id, nil)
  ...
  def list_jobs(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [data_set_id: nil, max_results: nil, next_token: nil, revision_id: nil
    # ])

    headers = []
    query_params = []

    {revision_id, options} = Keyword.pop(options, :revision_id, nil)
