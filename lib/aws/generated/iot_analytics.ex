...
defmodule AWS.IoTAnalytics do
  ...
  * `:include_statistics` (`t:boolean`) If true, additional statistical information about the channel is included in the response.
  ...
  def describe_channel(%Client{} = client, channel_name, options \\ []) do
    ...
    # options = Keyword.validate!(options, [include_statistics: nil
    # ])

    headers = []
    query_params = []

    {include_statistics, options} = Keyword.pop(options, :include_statistics, nil)
  ...
  * `:scheduled_on_or_after` (`t:timestamp`) A filter to limit results to those dataset contents whose creation is scheduled on or
  ...
  def list_dataset_contents(%Client{} = client, dataset_name, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [max_results: nil, next_token: nil, scheduled_before: nil, scheduled_on_or_after: nil
    # ])

    headers = []
    query_params = []

    {scheduled_on_or_after, options} = Keyword.pop(options, :scheduled_on_or_after, nil)
    ...
    {scheduled_before, options} = Keyword.pop(options, :scheduled_before, nil)
  ...
  * `:start_time` (`t:timestamp`) The start of the time window from which sample messages are retrieved.
  ...
  def sample_channel_data(%Client{} = client, channel_name, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [end_time: nil, max_messages: nil, start_time: nil
    # ])
    ...
    {max_messages, options} = Keyword.pop(options, :max_messages, nil)
