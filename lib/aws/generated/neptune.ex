...
defmodule AWS.Neptune do
  ...
  respective query languages Apache TinkerPop Gremlin and SPARQL, allowing you
  ...
  * `:input` (`t:create_db_subnet_group_message`)
  ...
  can specify the type of source (SourceType) you want to be notified of,
  ...
  * `:input` (`t:delete_db_cluster_snapshot_message`)
  ...
  and can't be recovered. Manual DB snapshots of the DB instance to be deleted
  by `DeleteDBInstance` are not deleted. If you request a final DB snapshot the
  ...
  * `:input` (`t:describe_db_cluster_parameters_message`)
  ...
  list of IDs for the Amazon accounts that are authorized to copy or restore the
  ...
  * `:input` (`t:modify_db_cluster_parameter_group_message`)
  ...
  Amazon accounts, specify `restore` as the `AttributeName` and use the
  `ValuesToAdd` parameter to add a list of IDs of the Amazon accounts that are
  authorized to restore the manual DB cluster snapshot. Use the value `all` to
  make the manual DB cluster snapshot public, which means that it can be copied
  or restored by all Amazon accounts. Do not add the `all` value for any manual
