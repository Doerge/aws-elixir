...
defmodule AWS.KafkaConnect do
  ...
  * `:next_token` (`t:string`) If the response of a ListConnectors operation is truncated, it will include a NextToken.
  ...
  def list_connectors(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [connector_name_prefix: nil, max_results: nil, next_token: nil
    # ])
    ...
    {connector_name_prefix, options} = Keyword.pop(options, :connector_name_prefix, nil)
