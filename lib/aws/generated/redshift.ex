...
defmodule AWS.Redshift do
  ...
  * `:input` (`t:associate_data_share_consumer_message`)
  ...
  or an Amazon EC2 instance, you can authorize inbound access to either a
  Classless Interdomain Routing (CIDR)/Internet Protocol (IP) range or to an
  Amazon EC2 security group. You can add as many as 20 ingress rules to an
  Amazon Redshift security group. If you authorize access to an Amazon EC2
  ...
  * `:input` (`t:create_endpoint_access_message`)
  ...
  subscribe to the topic. The ARN is displayed in the SNS console. You can
  ...
  you want that match those criteria. For example, you can specify source type =
  ...
  * `:input` (`t:delete_authentication_profile_message`)
  ...
  in the *Amazon Redshift Cluster Management Guide*. If you want to shut down
  the cluster and retain it for future use, set *SkipFinalClusterSnapshot* to
  `false` and specify a name for *FinalClusterSnapshotIdentifier*. You can later
  ...
  * `:input` (`t:describe_cluster_db_revisions_message`)
  ...
  family name. You can optionally specify a name to retrieve the description of
  a specific parameter group. For more information about parameters and
  ...
  * `:input` (`t:describe_cluster_parameters_message`)
  ...
  only that security group. For information about managing security groups, go
  ...
  * `:input` (`t:describe_cluster_security_groups_message`)
  ...
  of all clusters that are owned by your Amazon Web Services account. No
  ...
  * `:input` (`t:describe_cluster_snapshots_message`)
  ...
  your cluster subnet groups. By default, this operation returns information
  ...
  * `:input` (`t:describe_cluster_versions_message`)
  ...
  and access properties. This operation supports pagination. For more
  ...
  keys and tag values in the same request, Amazon Redshift returns all clusters
  that match any combination of the specified keys and values. For example, if
  ...
  * `:input` (`t:describe_events_message`)
  ...
  certificates owned by your Amazon Web Services account. If you specify both
  ...
  * `:input` (`t:describe_node_configuration_options_message`)
  ...
  Availability Zones (AZ) in the specific Amazon Web Services Region that you
  can specify, and the node types you can request. The node types differ by
  ...
  * `:input` (`t:%{}`)
  ...
  the `TableRestoreRequestId` parameter, then `DescribeTableRestoreStatus`
  ...
  * `:input` (`t:failover_primary_compute_input_message`)
  ...
  name prefixed with `IAM:` if `AutoCreate` is `False` or `IAMA:` if
  `AutoCreate` is `True`. You can optionally specify one or more database user
  groups that the user will join at log on. By default, the temporary
  credentials expire in 900 seconds. You can optionally specify a duration
  between 900 seconds (15 minutes) and 3600 seconds (60 minutes). For more
  ...
  * `:input` (`t:get_cluster_credentials_message`)
  ...
  Returns a database user name and temporary password with temporary authorization
  to log in to an Amazon Redshift database. The database user is mapped 1:1 to
  the source Identity and Access Management (IAM) identity. For more information
  about IAM identities, see [IAM Identities (users, user groups, and
  roles)](https://docs.aws.amazon.com/IAM/latest/UserGuide/id.html) in the
  Amazon Web Services Identity and Access Management User Guide. The Identity
  and Access Management (IAM) identity that runs this operation must have an IAM
  policy attached that allows access to all necessary actions and resources. For
  ...
  * `:input` (`t:resize_cluster_message`)
  ...
  the default cluster security and parameter groups. After Amazon Redshift
