...
defmodule AWS.IoTEvents do
  ...
  * `:alarm_model_version` (`t:string`) The version of the alarm model.
  ...
  def describe_alarm_model(%Client{} = client, alarm_model_name, options \\ []) do
    ...
    # options = Keyword.validate!(options, [alarm_model_version: nil
    # ])

    headers = []
    query_params = []

    {alarm_model_version, options} = Keyword.pop(options, :alarm_model_version, nil)
  ...
  * `:detector_model_version` (`t:string`) The version of the detector model.
  ...
  def describe_detector_model(%Client{} = client, detector_model_name, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [detector_model_version: nil
    # ])

    headers = []
    query_params = []

    {detector_model_version, options} = Keyword.pop(options, :detector_model_version, nil)
