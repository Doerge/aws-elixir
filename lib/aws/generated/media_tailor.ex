...
defmodule AWS.MediaTailor do
  ...
  * `:next_token` (`t:string`) (Optional) If the playback configuration has more
  ...
  def get_channel_schedule(%Client{} = client, channel_name, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [audience: nil, duration_minutes: nil, max_results: nil, next_token: nil
    # ])
    ...
    {duration_minutes, options} = Keyword.pop(options, :duration_minutes, nil)
    ...
    {audience, options} = Keyword.pop(options, :audience, nil)
