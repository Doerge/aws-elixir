...
defmodule AWS.KinesisVideoSignaling do
  ...
  is used to improve the connectivity of peer-to-peer applications. By providing
  a cloud-based relay service, TURN ensures that a connection can be established
  even when one or more peers are incapable of a direct peer-to-peer connection.
  For more information, see [A REST API For Access To TURN
  ...
  ## Optional parameters:
  """
  @spec get_ice_server_config(AWS.Client.t(), get_ice_server_config_request(), Keyword.t()) ::
  ...
  ## Optional parameters:
  """
  @spec send_alexa_offer_to_master(
          AWS.Client.t(),
          send_alexa_offer_to_master_request(),
          Keyword.t()
        ) ::
