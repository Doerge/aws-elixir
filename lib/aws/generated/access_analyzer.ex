...
defmodule AWS.AccessAnalyzer do
  ...
  **IAM User Guide**.
  ...
  ## Required positional parameters:
   • :analyzer_name (t:string String.t/0) (analyzerName)
  ...
  ## Required positional parameters:
   • :access_preview_id (t:string String.t/0) (accessPreviewId)

  ## Optional parameters:
   • :analyzer_arn (t:String.t/0) (analyzerArn)
  """
  ...
  def get_access_preview(%Client{} = client, access_preview_id, analyzer_arn, options \\ []) do
    ...
    # options = Keyword.validate!(options, [analyzer_arn: nil
    # ])

    headers = []
    query_params = []

    {analyzer_arn, options} = Keyword.pop(options, :analyzer_arn, nil)
  ...
  def get_analyzed_resource(%Client{} = client, analyzer_arn, resource_arn, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [analyzer_arn: nil, resource_arn: nil
    # ])
  ...
  def get_finding_v2(%Client{} = client, id, analyzer_arn, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [analyzer_arn: nil, max_results: nil, next_token: nil
    # ])
  ...
  ## Optional parameters:
   • :include_resource_placeholders (t:String.t/0) (includeResourcePlaceholders)
   • :include_service_level_template (t:String.t/0) (includeServiceLevelTemplate)
  """
  ...
  def get_generated_policy(%Client{} = client, job_id, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [include_resource_placeholders: nil, include_service_level_template: nil
    # ])

    headers = []
    query_params = []

    {include_service_level_template, options} =
      Keyword.pop(options, :include_service_level_template, nil)
    ...
    {include_resource_placeholders, options} =
      Keyword.pop(options, :include_resource_placeholders, nil)
  ...
  ## Optional parameters:
   • :max_results (t:String.t/0) (maxResults)
   • :next_token (t:String.t/0) (nextToken)
   • :principal_arn (t:String.t/0) (principalArn)
  """
  ...
  def list_policy_generations(%Client{} = client, options \\ []) do
    ...
    # NOTE: We can't use validate!/2 here because the user might pass options to the client too...
    # options = Keyword.validate!(options, [max_results: nil, next_token: nil, principal_arn: nil
    # ])

    headers = []
    query_params = []

    {principal_arn, options} = Keyword.pop(options, :principal_arn, nil)
